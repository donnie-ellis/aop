...
- **Cron:** robfig/cron v3
- **Agent dispatch:** REST via `AgentTransport` interface (`/controller/transport/rest.go`)
...
The controller never constructs HTTP requests to agents directly. All dispatch calls
go through the `AgentTransport` interface:
...
The v1 implementation (`RESTTransport`) lives in `/controller/transport/rest.go`. It
makes standard HTTP calls to the agent's long-poll endpoint and the cancel endpoint.

A future `GRPCTransport` would live in `/controller/transport/grpc.go` and implement
the same interface. The reconciliation loop never changes — it only ever calls
`transport.Dispatch()`. Switching transports is a config change and a new implementation
file, not a refactor.
- **Logging:** zerolog
...
- `AgentTransport` is an interface — gRPC transport is a new implementation in
  `/controller/transport/grpc.go`, the reconciliation loop never changes
...
- Scheduler change detection is live — no restart required for schedule updates